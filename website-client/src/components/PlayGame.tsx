...
import {parseCommand, updateState} from './Controller/parse-command'
import MacroRecorder from './Macros/MacroRecorder'
import Macros from './Macros/Macros'
import {PlayGameProps, PlayGameState} from '../types'
...
class PlayGame extends React.Component<PlayGameProps, PlayGameState> {
    ...
    private handleGamepadConnected(e: any | GamepadEventInit): void {
        console.debug("Gamepad connected at index %d: %s. %d buttons, %d axes.", e.gamepad.index, e.gamepad.id, e.gamepad.buttons.length, e.gamepad.axes.length)
        this.state.inputMethod.stop()
        // Fix: Added non-null assertion for controllerState
    ...
    private handleGamepadDisconnected(e: any | GamepadEventInit): void {
        console.debug("Gamepad disconnected at index %d: %s.", e.gamepad.index, e.gamepad.id)
    ...
    private handleChange(event: React.ChangeEvent<HTMLInputElement>) {
        // Fix: Spread existing state so typing doesn't reset the whole app
        this.setState({
            ...this.state,
        ...
        return (<Container>
            <Grid className={classes.connectionSection} container spacing={3}>
                <Grid item xs={12} sm={6}>
                    <TextField className={classes.serverAddressInput} label="Server Address" name="serverAddress"
                               value={this.state.serverAddress} onChange={this.handleChange}/>
                ...
                <Grid item xs={12} sm={6}>
                    <Button variant="contained" onClick={this.toggleSendMode}
                            disabled={!this.state.socket || !this.state.socket.connected}
                ...
                <Grid item xs={12} sm={6} hidden>
                    <Typography component="p">Click this button if you want to attempt to redo the pairing of the
                        simulated controller and the Switch.</Typography>
                    <Button variant="contained" onClick={this.reconnectSwitch}
                            disabled={!this.state.socket || !this.state.socket.connected}>
                        Reconnect Switch
                    </Button>
                </Grid>
            ...
            <div className={classes.infoSection}>
                <Typography component="p">
                    The original source code for this site can be found at <Link
                    href="https://github.com/juharris/switch-remoteplay"
                    target="_blank">https://github.com/juharris/switch-remoteplay</Link>.
                </Typography>
            </div>
        </Container>)
    }
